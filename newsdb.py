...
def get_3_most_popular_articles():
    ...
    c = db.cursor()
    c.execute("SELECT a.title, COUNT(a.id) AS count FROM articles a JOIN log l ON (l.path ILIKE '%' || a.slug || '%') GROUP BY a.id ORDER BY count DESC LIMIT 3")
    most_popular_3articles = c.fetchall()
    db.close()
    print("Most popular 3 articles of all time: \n")
    x = 0
    while x < len(most_popular_3articles):
        print("\"" + str(most_popular_3articles[x][0]) + "\"" + " - " + str(most_popular_3articles[x][1]) + " views")
        x = x+1
...
def get_authors_with_most_views():
    ...
    db = psycopg2.connect(database=DBNAME)
    c = db.cursor()
    c.execute("SELECT auth.name, COUNT(auth.id) AS views FROM authors auth JOIN articles a ON (a.author = auth.id) JOIN log l ON (l.path ILIKE '%' || a.slug || '%') GROUP BY auth.id ORDER BY views DESC;")
    authors_most_views = c.fetchall()
    db.close()
    print("\nMost popular article authors of all time: \n")
    x = 0
    while x < len(authors_most_views):
        print(str(authors_most_views[x][0]) + "\"" + " - " + str(authors_most_views[x][1]))
        x = x+1
...
def get_days_errors_higher_1percent():
    ...
    db = psycopg2.connect(database=DBNAME)
    c = db.cursor()
    c.execute("SELECT day, errorfraction * 100 AS percentage FROM (SELECT time::DATE AS day, (COUNT(CASE WHEN status <> '200 OK' THEN 1 END)::decimal) / (COUNT(*)::decimal) as errorfraction FROM log GROUP BY day) AS errorsperday WHERE errorfraction > 0.01;")
    ...
    while x < len(errors_higher_1percent):
        format_date = convert_date(errors_higher_1percent[x][0])
        percent = round(errors_higher_1percent[x][1], 2)
        print(str(format_date) + " - " + str(percent))
        x = x+1
